c
c DECOMP(SIGNAL,NT,NX,DT,DX,FILTER,SIGNALW,SIGNALS,SIGNALE)
c
c This subroutine splits the matrix SIGNAL(NT,NX) into its 
c upward/stationary/downward components SIGNALW(NT,NX), 
c SIGNALS(NT,NX) and SIGNALE(NT,NX), given space/time 
c resoultions DX/DT.
c Parameter FILTER is a 4 real values matrix containing the
c space and time limits in order to do a filtering:
c FILTER(XMIN,XMAX,TMIN,TMAX)
c The methos used is a 2D Fourier analysis (with subroutine 
c FOURT inot Bfft.f).
c For more details, check:
c http://scripts.mit.edu/~gmaze/blog/?page_id=124
c
c Guillaume MAZE 
c 2004, gmaze@mit.edu
c
c Bibliography: 
c Park, Y.-H. (1990).
c   Mise en évidence d'ondes planétaires semi-annuelles 
c   baroclines au sud de l'océan indien par altimétrie satellitaire.
c   C. R. Acad. Sci. Paris, 310(2):919-926.
c
c Park, Y.-H., Roquet, F., and Vivier, F. (2004).
c   Quasi-stationary enso wave signals versus the antarctic 
c   circumpolar wave scenario.
c   Geophys. Res. Lett., 31(L09315). 
c
c-------------------------------------------------------------
      subroutine decomp(signal,nt,nx,dt,dx,
     &                  filter,
     &                  signalW,signalS,signalE)
c-------------------------------------------------------------
c
c
...
c Initialise indices and matrices
c
      nft=nt/2
      nfx=nx/2
      pi=4*atan(1.)
      ...
      enddo
c
c Eventualy produce a test signal:
      if(0.EQ.1) then
      do it=1,nt
         do ix=1,nx
...
c
c
c Frequencies (space and time) to be conserved:
      xmin=filter(1)
      xmax=filter(2)
      tmin=filter(3)
      tmax=filter(4)
      ...
      do ifx=1,nfx
         do ift=1,nft
            if( (ca(ifx).LE.camax).AND.
     &          (ca(ifx).GE.camin).AND.
     &          (omega(ift).LE.omegamax).AND.
     &          (omega(ift).GE.omegamin) ) then
                compose(ift,ifx) = 1.
            else
                compose(ift,ifx) = 0.
            endif
...
c Coef of FFT in space for each time step:
c ------------------------------------------------------
      do it=1,nt
...
c Coef of FFT in time of space-coef:
c ------------------------------------------------------
      do ifx=1,nfx
         ! For coef Ck:
         do it=1,nt
...
c
c
c Coef of cos/sin for down/up-ward directions
c Amplitude and phase of components down/up
c ------------------------------------------------------
      ...
      do ift=1,nft
       ...
       enddo
c
c Reconstruction of splited signals
c ------------------------------------------------------
             ...
             do ifx=1,nfx
                do ift=1,nft
                   ...
                   if (Aw.GT.Ae) then  ! If upward higher than downward
                      ! upward wave phase:
                      pxt3 =   ca(ifx)*xx(ix)
     &                       + omega(ift)*tt(it) - phiw
                      ! the signal:
                      varS = varS + compo*
     &                       2*Ae*cos(pxt1)*cos(pxt2)
                      varW = varW + compo*(
     &                       (Aw-Ae)*cos(pxt3))
                   else ! otherwise downward higher than upward
                      ! downward wave phase:
                      pxt3 =   ca(ifx)*xx(ix)
     &                       - omega(ift)*tt(it) - phie
                      ! the signal:
                      varS = varS + compo*
     &                       2*Aw*cos(pxt1)*cos(pxt2)
                      varE = varE + compo*(
     &                       (Ae-Aw)*cos(pxt3))
                   endif
                enddo !ift
