...
%   $Revision: 5.11 $  $Date: 2002/03/14 23:18:31 $
...
if ~isreal(x) & nargout>1
   error('Cannot compute p-values for complex inputs.');
end
...
if ~somenan | ~isequal(userows,'pairwise') | size(x,2)<2
...
if nargout>=2
   ...
   t = denom>0;
   rv = rv(t);
   if length(n)>1, nv = nv(t); end
   Tstat(t) = rv .* sqrt((nv-2) ./ denom(t));
   p = zeros(m);
...
if ~isnumeric(alpha) | prod(size(alpha))~=1 | alpha<=0 | alpha>=1
