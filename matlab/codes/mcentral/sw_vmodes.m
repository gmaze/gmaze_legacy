...
function [Vert, Hori, Edep, PVel, z] = sw_vmodes(z,N,clat,nmodes);
...
if isempty(good)
	warning('Not enough points to compute vertical modes for this profile');
	Vert = zeros(length(z),nmodes)*NaN;
	Hori = zeros(length(z),nmodes)*NaN;
	Edep = zeros(1,nmodes)*NaN;
	PVel = zeros(1,nmodes)*NaN;
	return;
else
	N = N(good); 
	z = z(good);
