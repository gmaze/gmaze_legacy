...
%POLARDENDROGRAM plots a polar mydendrogram plot, taking same options as
%mydendrogram and giving same outputs.
% Example:
% X= rand(100,2);
% Y= pdist(X,'cityblock');
% Z= linkage(Y,'average');
% [H,T] = polarmydendrogram(Z,'colorthreshold','default');

%Plot a normal mydendrogram
[h,T,perm] = mydendrogram(Z,varargin{:});
