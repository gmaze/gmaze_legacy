...
Version 1.4.8 Aug 30, 2007, (c) Peter Jipsen http://www.chapman.edu/~jipsen
...
var mathcolor = "darkred";        // change it to "" (to inherit) or any other color
var mathfontsize = "1.2em";       // change to e.g. 1.2em for larger math
var mathfontfamily = "serief";  // change to "" to inherit (works in IE) 
...
var AMdocumentId = "content" // id for element containing math (default=body)
...
// Replaced AMparseMath from ASCIIMathML 2.09
// Mohammad Rahmani
...
//tokens are single letters (except a, A, I) and ASCIIMathML tokens
  ...
  var secondenglishAMtoken = "|\\bI\\b|\\bin\\b"; // took if and or not out
  ...
  var re1 = new RegExp("(^|\\s)([b-zB-HJ-Z]|\\d+|"+texcommand+ambigAMtoken+simpleAMtoken+")(\\s|\\n|$)","g");
  var re2 = new RegExp("(^|\\s)([a-z]|\\d+|"+texcommand+ambigAMtoken+simpleAMtoken+")([,.])","g");
  ...
  str = str.replace(/`(\((a\b|in\b))(.*?[a-zA-Z]{2,}\))/g,"$1`$3)");  //fix parentheses
  str = str.replace(/`(\(\w\)[,.]?)/g,"$1`");//[,.](\s|$))/g,"$1`");
  str = str.replace(/`([0-9.]+:)`/g,"$1");
  return str;
}

function AMprocessNodeR(n, linebreaks) {
  var mtch, str, arr, frg, i, re1,re2;
  if (n.childNodes.length == 0) {
   if ((n.nodeType!=8 || linebreaks) &&
    ...
    if (!(str == null)) {
      ...
      str = str.replace(/\s*\r\n/g," ");
      re1 = new RegExp(AMescape1, "g");
      re2 = new RegExp(AMescape2, "g");
      mtch = re1.test(str) || re2.test(str);
      if (!mtch) mtch = /^(Theorem|Lemma|Proposition|Corollary|Definition|Example|Remark|Problem|Exercise|Conjecture|Solution|Proof|QED)/g.test(str);
      if (!mtch) {
        mtch = /AMath/.test(str);
        if (mtch) str = str.replace(/AMath/,"");
      }
...
function AMprocessNode(n, linebreaks, spanclassAM) {
  ...
  if (spanclassAM!=null) {
    ...
    if (st==null || st.indexOf("``")!=-1 || st.indexOf("AMath")!=-1 ||
      st.indexOf(AMdelimiter1+" ")!=-1 || st.indexOf(AMdelimiter2+" ")!=-1 ||
      st.indexOf(AMdelimiter1+"<")!=-1 || st.indexOf(AMdelimiter1+"\n")!=-1) {
      if (st.indexOf("AMath")!=-1 || st.indexOf(" `` ")!=-1) automathrecognize = true;
      if (false && st!=null && automathrecognize) {
        st = st.replace(/(Proof:)/g,"<i>$1</i>");
        st = st.replace(/QED/g,"&#x25A1;");
        n.innerHTML = st.replace(/>(Theorem|Lemma|Proposition|Corollary|Definition|Example|Remark|Problem|Exercise|Conjecture|Solution)(:|\W\W?\w*?\W?:)/g,"><b>$1$2</b>");
      }
