		...
		if fid > 0
			...
			while 1
				...
				if ~isempty(strfind(tline,pate))
					...
					ii = max(strfind(tline,pate));
					iter = iter + 1;
					tim = tline(ii+length(pate):end);
					if strfind(tim,'*');
						et(iter) = et(iter-1);
					else
						et(iter) = str2num(tim);
		...
		if fid > 0			
			...
			while 1
				...
				if ~isempty(strfind(tline,'=>'))
					iter = iter + 1;
					et(iter) = str2num(tline(strfind(tline,'=>')+3:strfind(tline,'sec')-1));
				end
			end
		end
		nt = 1099;
